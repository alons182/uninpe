...
class JoomdleControllerCertificate extends JController
{
	...
	function send_certificate ()
	{
        ...
        $pdf =file_get_contents ($moodle_url.'/auth/joomdle/certificate_view.php?id='.$cert_id.'&certificate=1&action=review&username='.$username.'&token='.$token);
		echo $moodle_url.'/auth/joomdle/certificate_view.php?id='.$cert_id.'&certificate=1&action=review&username='.$username.'&token='.$token;
		exit ();
